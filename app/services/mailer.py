...
<html>
  <body style="margin:0;padding:0;background:#FAF5EE;font-family:'Nunito Sans',Verdana,sans-serif;color:#2B2622;">
    <div style="max-width:560px;margin:0 auto;padding:24px;">
      <div style="background:#4A2C3F;color:#FAF5EE;border-radius:16px 16px 0 0;padding:20px 28px;
...
<p style="text-align:center;margin:28px 0;">
  <a href="{url}" style="background:#D9A441;color:#2B2622;text-decoration:none;font-weight:700;
...
def send_login_link(to: str, link: str) -> bool:
    minutes = current_app.config["MAGIC_LINK_MAX_AGE_MINUTES"]
    text = (
    ...
    html = (
        "<p>Hi,</p><p>Tap the button below to sign in \u2014 no password needed.</p>"
        + _BUTTON.format(url=link, label="Sign me in")
        + f"<p style='color:#6B6159;font-size:14px;'>This link expires in {minutes} minutes. "
        "If you didn't request it, you can ignore this email.</p>"
    )
    return send_email(to, "Your login link", text, html)
